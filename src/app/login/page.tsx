...
function LoginForm() {
  ...
  const saveExamPreferences = async () => {
    ...
    const existingAgencies = isGoogleNewUser ? selectedAgencyIds : user?.agencies || selectedAgencyIds;
    try {
          ...
          <div className="bg-card w-full max-w-lg max-h-[85vh] overflow-y-auto rounded-3xl border border-border shadow-2xl flex flex-col">
            <div className="p-6 pb-4 border-b border-border">
              <h2 className="text-lg font-bold font-outfit flex items-center gap-2">
                ...
                {isGoogleNewUser ? 'Welcome! Set Your Preferences' : 'Choose Your Exams'}
              </h2>
              <p className="text-xs text-muted-foreground mt-1.5">
                Select the agencies and exams you want to prepare for. These will determine
                which test series appear on your dashboard.
              </p>
            </div>

            <div className="flex-1 overflow-y-auto p-6">
              {/* Step 1: Agencies (Google new-user flow) */}
              {isGoogleNewUser && (
                <div className="flex flex-col gap-1.5 mb-5">
                  ...
                  {selectedAgencyIds.length > 0 && (
                    <button
                      onClick={() => fetchExamsForAgencies(selectedAgencyIds)}
                      className="self-start mt-1 px-3 py-1.5 rounded-lg bg-secondary text-xs font-semibold hover:bg-secondary/80"
                    >
                      Load exams for selected agencies
                    </button>
                  )}
                </div>
              )}
